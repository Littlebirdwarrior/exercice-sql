...
SELECT COUNT(id_personnage) FROM `personnage` WHERE id_lieu = '1' --compte seulement village gaulois
SELECT COUNT(id_personnage) FROM `personnage` GROUP BY `id_lieu`; --compte les perso sans afficher le lieu, et sans trier

SELECT `id_lieu`,
COUNT(id_personnage) AS nombre_personnage 
FROM `personnage` 
GROUP BY `id_lieu` 
ORDER BY nombre_personnage
;--classe nombre_personnage par id_lieu dans ordre croissant, mais sans je dois encore récupérer les nom
...
SELECT  nom_lieu ,nom_personnage, adresse_personnage, nom_specialite
FROM personnage
...
; -- affiche les nom, les personnage et les lieu

SELECT  nom_lieu ,nom_personnage, adresse_personnage, nom_specialite
FROM personnage JOIN specialite JOIN lieu  -- !!NE MARCHE PAS, n'affiche que le nom d'Asterix
...
INNER JOIN lieu ON lieu.id_lieu = personnage.id_lieu
INNER JOIN specialite ON specialite.id_specialite = personnage.id_specialite
GROUP BY personnage.id_lieu --affiche bien ce qui est voulu, mais les lieu sont sur une seule ligne et les élément concaténée
...
SELECT nom_bataille, DATE_FORMAT(`date_bataille` , "%D %b %y") as date_bataille_formatee
FROM `bataille` --selectionne et formate la date de la bataille (%y = 2 digit)
...
SELECT nom_potion, nom_ingredient, cout_ingredient
FROM composer
INNER JOIN potion ON potion.id_potion = composer.id_potion
INNER JOIN ingredient ON ingredient.id_ingredient = composer.id_ingredient --affiche et les potions et les ingredients à partir de la table composer, mais pas de relation logique entre eux

SELECT potion.nom_potion, 
GROUP_CONCAT(ingredient.nom_ingredient SEPARATOR ' | ') AS ingredients, 
SUM(composer.qte) AS quantites
FROM potion
...
GROUP BY(composer.id_potion)--affiche les nom de potions, et sur une ligne le nom des ingredients
...
GROUP BY -- evite d'avoir 4 fois la ligne
	bataille.id_bataille -- affiche le nom de bataille et la quantité mais pas le nom
...
ORDER BY 
    quantite_casque DESC --affiche les bataille et la quantité de casque prise par ordre décroissant
...
SELECT
	...
	GROUP_CONCAT(casque.cout_casque SEPARATOR ' | ') AS cout_unitaire, --evite ecrasement des valeurs
	GROUP_CONCAT(prendre_casque.qte SEPARATOR ' | ') AS quantite_casque
FROM 
	casque
	INNER JOIN prendre_casque ON prendre_casque.id_casque = casque.id_casque
GROUP BY
	casque.id_type_casque --permet d'afficher toutes les infos pour chaque type (nom, chaque cout, chaque quantité)


SELECT
	casque.id_type_casque,
	GROUP_CONCAT(casque.nom_casque SEPARATOR ' | '),
	SUM(casque.cout_casque) AS cout_unitaire_chaque_nom,
	SUM(prendre_casque.qte) AS quantite_casque
FROM 
	casque
	INNER JOIN prendre_casque ON prendre_casque.id_casque = casque.id_casque
GROUP BY
	casque.id_type_casque -- fait la somme des quantité et des cout unitaire de chaque nom 
...
FROM 
	...
	INNER JOIN composer ON potion.id_potion = composer.id_potion
	INNER JOIN ingredient ON ingredient.id_ingredient = composer.id_ingredient
GROUP BY potion.nom_potion -- affiche toutes les potions avec le nom de leurs ingrédients
